...
### Fault tolerance
 ...
 NameNode - это SPOF(Single point of failure) в HDFS, потому что падение NameNode, приводит к тому, что вся система перестаёт работать. Соответственно она должна работать на отдельной надёжной машине.

В последних версиях hadoop, сделали "правильную" Secondary NameNode(Active Standby), который в случае падения NameNode берёт всю нагрузку на себя. При этом все данные, которые были на NameNode он копирует себе.
